            ...
            <v-container fluid grid-list-xl>
                <v-layout row wrap>
                    <v-flex xs12>
                        ...
                        <v-btn color="primary" @click="newRuleset">Add New Rulset</v-btn>
                    </v-flex> 
                    ...
                    <!-- <v-flex xs12 sm6 >
                        <h4>Edit Rules</h4> 
                        <h3 v-show="editRules.edit.condition.length">If</h3>
                        <ul>
                            <li v-for="(part,index) in editRules.edit.condition" :key="index">
                                {{ part.attribute | formatEditAttribute }} is {{ part.operator | formatEditOperator }} {{ part.value }}
                                <span class="font-weight-bold" v-if="index < editRules.edit.condition.length - 1">AND</span>  
                                <v-icon color="error" small class="pointer" @click="removeCondition(index)">delete</v-icon>  
                            </li>
                        </ul>
                        <h3 v-show="editRules.edit.change.length">Then</h3>
                        <ul>
                            <li v-for="(part,index) in editRules.edit.change" :key="index">
                                {{ part.attribute | formatEditAttribute }} is {{ part.operator | formatChangeOperator }} {{ part.value }}
                                <span class="font-weight-bold" v-if="index < editRules.edit.change.length - 1">AND</span>  
                        ...
                        <v-flex xs12 v-show="rulesetBuilder">
                            <v-card>  
                                <v-toolbar>
                                    ...
                                    <v-spacer/> 
                                    <v-btn icon @click="conditionBuilderSubmit($v)">
                                        <v-icon>add_to_photos</v-icon>
                                    </v-btn>
                                    <v-btn icon @click="closeConditionBuilder()">
                                        <v-icon>close</v-icon>
                                    </v-btn>
                                </v-toolbar>
                            ...
                            <v-card>  
                                <v-toolbar>
                                    ...
                                    <v-spacer/> 
                                    <v-btn icon @click="conditionChangeBuilderSubmit($v)">
                                        <v-icon>add_to_photos</v-icon>
                                    </v-btn>
                                    <v-btn icon @click="closeConditionChangeBuilder()">
                                        <v-icon>close</v-icon>
                                    </v-btn>
                                </v-toolbar>
...
export default {
    ...
    methods:{
        ...
        conditionChangeBuilderSubmit($v){
            ...
            if (isValid) {
                ...
                this.editRules.edit.change.push(condition);
        ...
        conditionBuilderSubmit($v){
            ...
            if (!this.$v.conditionEditAttribute.$invalid && !this.$v.conditionEditOperator.$invalid && !this.$v.conditionEditValue.$invalid) {
                ...
                this.editRules.edit.condition.push(condition);
        ...
        removeCondition(index){ 
			this.editRules.edit.condition.splice(index,1); 
        },
        removeChange(index){
			this.editRules.edit.change.splice(index,1);  
        },
        newRuleset(){
    ...
    computed:{  
        userChangeAttributes(){
            ...
            var selectedLength = this.editRules.edit.change.length;

            for (let index = 0; index < selectedLength; index++) {
                const element = this.editRules.edit.change[index];
        ...
        /* conditionOperatorAttributesDb(){
            var db = {};
            var length = conditionOperatorAttributes.length
            for (let index = 0; index < length; index++) {
                const element = conditionOperatorAttributes[index];
                db
